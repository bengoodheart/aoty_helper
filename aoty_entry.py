...
def inputter():
    ...
    while(q == False):
            ...
            #Pre-write summary
            print("Entry recorded:", album, "by", artist,"| Genre:", genre,"| Label:", label, "| You rated it a ",rating,"/10")
            print("You commented: ", comments)
            correct = input("Is this correct? y/n: ")
            if (correct.lower() != 'n'):
                writer.writerow([artist, album, genre, label, rating, comments])
            else:
                writer.writerow([])
                continue
...
if fileExists('aoty.csv'):
    ...
    with open('aoty.csv', 'w') as csv_file:
        writer = csv.writer(csv_file, quoting=csv.QUOTE_ALL)
        writer.writerow(['Arist','Album Name','Genre', 'Label', 'Rating', 'Comments'])
        inputter()
